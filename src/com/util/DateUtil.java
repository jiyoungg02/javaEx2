...
public class DateUtil {
	...
	public boolean isValidDate2(String dateString) {
		String[] dateFormats = {"yyyy-MM-dd", "yyyy.MM.dd", "yyyy/MM/dd", "yyyyMMdd"};
		
		for(String format : dateFormats) {
			SimpleDateFormat sdf = new SimpleDateFormat(format);
			sdf.setLenient(false); // 엄격한 날짜 검사
				// 엄격한 날짜 검사에서는 2025-04-31 false
	...
	public Date toDate(String dateString) {
		...
		try {
			if(!isValidDate2(dateString)) {
				// 날짜형식이 잘못된 경우 예외발생
				throw new RuntimeException("날짜형식이 올바르지 않습니다.(" + dateString + ")");
			}
	...
	public String toDayOfWeek(String dateString) {
		...
		try {
			...
			SimpleDateFormat sdf = new SimpleDateFormat("E"); // E:요일
			result = sdf.format(d); // 일, 월, ...,토
