...
public class Ex01_ClassTypeCast {
	public static void main(String[] args) {
		...
		*/
		...
		Test1 t = new Test1();
		t.print(); // 10 : 20
		// System.out.println(t.c); // 컴오류. 하위클래스의 멤버 접근불가
		System.out.println();
		...
		*/
		...
		System.out.println(t1.b); // 20
			// 업캐스팅 객체라 할지라도 필드는 자기 자신것을 가리킨다.
		// System.out.println(t1.c); // 컴파일 오류
			// t1이 dm(하위클래스객체)를 참조하고 있지만
			// 컴파일러 관점에서는 t1에 존재한는 필드가 아니므로 에러
		
		t1.print();
			// - 업캐스팅한 객체에서 재정의한 메소드는 숨어버리기 때문에 
			//   하위 클래스의 메소드가 호출된다.
			// - 이곳에서는 t1 객체를 이용하여 Test1 클래스의 print()를 
			//   호출할 수 있는 방법이 없다.
		System.out.println();
		
		// t1.write(); // 컴파일 오류
			// - t1이 업캐스팅 객체라 할지라도 Test1클래스에
